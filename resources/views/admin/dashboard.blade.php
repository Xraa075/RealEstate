<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <h3 class="text-lg font-semibold mb-4">Monthly Asset Value Added ({{ $year }})</h3>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <a href="{{ route('admin.assets.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            {{ __('Add New Asset') }}
                        </a>
                        ...
                        <a href="{{ route('admin.asset-requests.index', ['status' => 'pending']) }}" class="inline-flex items-center px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                                ...
                                @forelse ($recentAssets as $asset)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <a href="{{ route('admin.assets.edit', $asset) }}" class="text-green-600 hover:text-green-900">Edit</a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                    ...
                    <div class="mt-4">
                        <a href="{{ route('admin.assets.index') }}" class="text-indigo-600 hover:text-indigo-900">View All Assets</a>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($recentRequests as $request)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <a href="{{ route('admin.asset-requests.show', $request) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                            @if($request->status === 'pending')
                                                <form method="POST" action="{{ route('admin.asset-requests.approve', $request) }}" class="inline mr-2">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="text-green-600 hover:text-green-900" onclick="return confirm('Approve this request?')">Approve</button>
                                                </form>
                                                <form method="POST" action="{{ route('admin.asset-requests.reject', $request) }}" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                    ...
                    <div class="mt-4">
                        <a href="{{ route('admin.asset-requests.index') }}" class="text-indigo-600 hover:text-indigo-900">View All Requests</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        new Chart(monthlyValueCtx, {
            type: 'bar',
            data: {
                labels: {!! json_encode(array_column($monthlyAssetValue, 'month')) !!},
                datasets: [{
                    label: 'Asset Value (IDR Billion)',
                    data: {!! json_encode(array_map(function($item) { return $item['value'] / 1000000000; }, $monthlyAssetValue)) !!},
