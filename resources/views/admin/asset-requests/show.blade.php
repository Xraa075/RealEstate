    ...
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('admin.asset-requests.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    {{ __('Back to List') }}
                </a>
            </div>
        </div>
                        ...
                        @if ($assetRequest->type === 'update' && $assetRequest->asset)
                            <div>
                                ...
                                <div class="border rounded-md p-4">
                                    ...
                                    <a href="{{ route('admin.assets.show', $assetRequest->asset) }}" class="text-indigo-600 hover:text-indigo-900 text-sm">View Asset Details</a>
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    @if ($assetRequest->status === 'pending')
                        <div class="mt-6 border-t pt-6">
                            ...
                            <div class="flex space-x-4">
                                <form action="{{ route('admin.asset-requests.approve', $assetRequest) }}" method="POST" class="flex-1">
                                    @csrf
                                    @method('PATCH')
                                    <div class="mb-4">
                                ...
                                <form action="{{ route('admin.asset-requests.reject', $assetRequest) }}" method="POST" class="flex-1">
                                    @csrf
                                    @method('PATCH')
                                    <div class="mb-4">
