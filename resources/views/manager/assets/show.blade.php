    ...
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('manager.assets.edit', $asset) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    {{ __('Edit') }}
                </a>
                <form action="{{ route('manager.assets.destroy', $asset) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                ...
                <a href="{{ route('manager.assets.index') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    {{ __('Back to List') }}
                </a>
            </div>
        </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <a href="{{ route('manager.assets.documents.create', $asset) }}" class="inline-flex items-center px-3 py-1 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Add Document
                            </a>
                        </div>
                        ...
                        @if($asset->documents->count() > 0)
                                ...
                                @foreach($asset->documents as $document)
                                    <li class="py-3">
                                        <div class="flex justify-between">
                                            ...
                                            <div class="flex space-x-2">
                                                <a href="{{ route('manager.assets.documents.download', [$asset, $document]) }}" class="text-indigo-600 hover:text-indigo-900" title="Download">
                            ...
                            <div class="mt-4">
                                <a href="{{ route('manager.assets.documents.index', $asset) }}" class="text-indigo-600 hover:text-indigo-900 text-sm font-medium">
