        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <!-- Search and Filter -->
                    <div class="mb-4">
                        <form action="{{ route('surveyor.requests.index') }}" method="GET" class="flex flex-wrap gap-4">
                            <div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($request->asset)
                                                {{ $request->asset->name }}
                                            @else
                                                <span class="text-gray-500">{{ $request->proposed_data['name'] ?? 'New Asset' }}</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap space-x-2">
                                            <a href="{{ route('surveyor.requests.show', $request) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                            @if ($request->status === 'pending')
                                                <a href="{{ route('surveyor.requests.edit', $request) }}" class="text-green-600 hover:text-green-900">Edit</a>
                                                <button onclick="confirmDelete('{{ $request->id }}')" class="text-red-600 hover:text-red-900">Delete</button>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 text-center">No asset requests found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <script>
        function confirmDelete(requestId) {
            if (confirm('Are you sure you want to delete this request? This action cannot be undone.')) {
                ...
                form.action = `/surveyor/requests/${requestId}`;
                form.submit();
            }
        }
    </script>
